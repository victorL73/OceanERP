...
public sealed record MailServerSettingsDto(Guid? Id, string SmtpHost, int SmtpPort, string ImapHost, int ImapPort, bool UseSsl, bool IsConfigured);
...
public sealed record EmailMessageDto(Guid Id, Guid? MailAccountId, string Subject, string From, string To, string Body, string Direction, string Status, bool IsRead, string? ErrorMessage, DateTimeOffset CreatedAt, DateTimeOffset? SentAt, DateTimeOffset? ReceivedAt, IReadOnlyList<EmailAttachmentDto> Attachments, IReadOnlyList<EmailLinkDto> Links);
...
public sealed record UpdateMailServerSettingsRequest(string SmtpHost, string ImapHost, int SmtpPort = 587, int ImapPort = 993, bool UseSsl = true);
